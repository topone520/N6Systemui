...
public class VtpHorTempSelectorView extends FrameLayout implements TouchEventListener {
    static String TAG = VtpHorTempSelectorView.class.getName();
    ...
    public VtpHorTempSelectorView(@NonNull Context context) {
        super(context);
    }

    public VtpHorTempSelectorView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this._context = context;
        initView();
    }

    public VtpHorTempSelectorView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        inflate = LayoutInflater.from(_context).inflate(R.layout.vtp_hor_temp_select_layout, null);
        addView(inflate);
        //温度滑块
        _hor_temp_view = findViewById(R.id.hor_temp_view);
        _hor_temp_view.setOtherTextSize(30);
        _hor_temp_view.setSelectTextSize(52);
        //温度减
    ...
    public void onTouchEvent(View view) {
        ...
        if (view.getId() == R.id.img_add) {
            ...
            _select_position--;
            if (_select_position < 0) _select_position = 0;
        } else if (view.getId() == R.id.img_reduce) {
            AAOP_LogUtils.d(TAG, "reduce ");
            _select_position++;
            if (_select_position > TR_ARRAY.length - 1) _select_position = TR_ARRAY.length - 1;
        }
        if (_listener != null)
            _listener.onSelectTemp(getTemp());

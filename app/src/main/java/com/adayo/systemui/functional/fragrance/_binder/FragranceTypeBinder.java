...
public class FragranceTypeBinder extends AbstractViewBinder<Integer> {

    private static final String TAG = FragranceTypeBinder.class.getSimpleName();
    ...
    private FragranceSlotView _slot_view; //香氛通道控件
    private RelativeLayout _frag_lin_error_view;

    public FragranceTypeBinder(HvacLayoutSwitchListener layoutSwitchListener, int position, int slotViewId, int linErrorId) {
    ...
    protected void _update_ui(Bundle bundle) {
        ...
        if (_slot_position == slot) {
            //存储香氛数据
            FragranceMapUtils.setType(_slot_position, type);
            //通道均不含香氛显示无香氛页面
            _frag_lin_error_view.setVisibility(FragranceMapUtils.isAllTypeNull() ? View.VISIBLE : View.GONE);
            if (FragranceDatabase.getInstance().fragranceDao().getFragranceInfoList().size() < 3) {
                //表中的数据不超过三条 说明为第一次使用 新增数据 记录香氛信息
                Log.d(TAG, "_update_ui: first use fragrance");
                FragranceDatabase.getInstance().fragranceDao().insertFragranceInfo(FragranceMapUtils.makeFragranceInfo(
                        _slot_position,
                        type,
                        ""));
            }
            int oldType;
            oldType = FragranceDatabase.getInstance().fragranceDao().getFragranceInfoByPosition(_slot_position).getType();
            if (oldType != type) {
                //香氛种类被更换 更新数据
                FragranceDatabase.getInstance().fragranceDao().updateFragranceInfo(FragranceMapUtils.makeFragranceInfo(
                        _slot_position,
                        type,
                        ""));
            }
            _slot_view.updateTypeUI(FragranceDatabase.getInstance().fragranceDao().getFragranceInfoByPosition(_slot_position));
            //判断自身香型是否为空 为空则显示下一个
            if (type == FragranceSOAConstant.TYPE_NULL) {
                int position = FragranceMapUtils.getDefaultXml();

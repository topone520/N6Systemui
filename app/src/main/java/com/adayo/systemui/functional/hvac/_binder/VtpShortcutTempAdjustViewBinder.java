...
import com.adayo.systemui.windows.views.hvac.VtpHorTempSelectorView;

public class VtpShortcutTempAdjustViewBinder extends AbstractViewBinder<Float> {
    private static final String TAG = VtpShortcutTempAdjustViewBinder.class.getSimpleName();
    private final int _position;
    private final int _resource_id;
    private VtpHorTempSelectorView _temp_select_view;

    public VtpShortcutTempAdjustViewBinder(ViewBinderProviderFloat provider, int position, int resource_id) {
        super(TAG, provider);
        _position = position;
    ...
    public void _bind_view(View view) {
        ...
        _temp_select_view.set_listener(temp -> {
            _set_value(temp);
        });
    }
    ...
    public static class Builder {
        ...
        private static IViewBinder createInstance(final int position, final Bundle bundle, int resource_id) {
            ViewBinderProviderFloat provider = (ViewBinderProviderFloat) new ViewBinderProviderFloat.Builder()
                    ...
                    .build();
            return new VtpShortcutTempAdjustViewBinder(provider, position, resource_id);
        }
    }

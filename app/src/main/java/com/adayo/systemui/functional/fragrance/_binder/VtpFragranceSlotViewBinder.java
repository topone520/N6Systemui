...
import com.adayo.systemui.windows.views.fragrance.VtpFragranceSlotBgView;
...
//香氛当前槽位&&香氛开关
public class VtpFragranceSlotViewBinder extends AbstractViewBinder<Integer> implements IChannelMonitor.IChannelObservable, ITitleMonitor.ITitleObservable {
    private static final String TAG = VtpFragranceSlotViewBinder.class.getSimpleName();
    private VtpFragranceSlotView _slot_view;
    private VtpFragranceSlotBgView _slot_bg_view;
    ...
    public VtpFragranceSlotViewBinder(IChannelMonitor monitor, int position, int slotViewId, int slotBgViewId) {
    ...
    protected void _update_ui(Bundle bundle) {
        ...
        try {
            ...
            _slot_bg_view.setUpdateViewUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(slot).getBackground());
            _slot_bg_view.setUpdateWriteUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(slot).getWrite());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void onChannelChanged(int channel, boolean isOpen) {
        ...
        //不存在的香氛类型
        _slot_bg_view.setUpdateViewUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(channel).getBackground());
        _slot_bg_view.setUpdateWriteUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(channel).getWrite());
    }
    ...
    public void onTitleChanged(int position) {
        ...
        //自定义香氛名称后立刻切换为默认背景
        try {
            _slot_bg_view.setUpdateViewUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(position).getBackground());
            _slot_bg_view.setUpdateWriteUI(FragranceDatabase.getInstance().vtpFragranceDao().getFragranceInfoByPosition(position).getWrite());
        } catch (Exception e) {
            e.printStackTrace();
        }

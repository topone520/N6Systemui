...
public class SeatVentilateViewBinder extends AbstractViewBinder<Integer> {
    ...
    private QsIconView _re_ventilate;
    ...
    private int _level;
    private int number;
    private static final HashSet<String> _hash_set = new HashSet<>(Arrays.asList(SeatSOAConstant.MSG_EVENT_SEAT_VENTILATION, SeatSOAConstant.MSG_EVENT_EXTREME_ENERGY, HvacSOAConstant.MSG_EVENT_POWER_VALUE));
    ...
    public static class Builder {
        public static IViewBinder createLeft(int resource_id, int[] level_resources) {
            Bundle bundle = new Bundle();
            bundle.putInt("target", SeatSOAConstant.LEFT_FRONT);
            return createInstance(SeatSOAConstant.LEFT_FRONT, bundle, resource_id, level_resources);
        }

        public static IViewBinder createRight(int resource_id, int[] level_resources) {
            Bundle bundle = new Bundle();
            bundle.putInt("target", SeatSOAConstant.RIGHT_FRONT);
            return createInstance(SeatSOAConstant.RIGHT_FRONT, bundle, resource_id, level_resources);
        }

        private static IViewBinder createInstance(final int position, final Bundle bundle, int resource_id, int[] level_resources) {
            ViewBinderProviderInteger provider = (ViewBinderProviderInteger) new ViewBinderProviderInteger.Builder()
                    ...
                    .withInitialValue(-1)
                    .build();
            return new SeatVentilateViewBinder(provider, position, resource_id, level_resources);
        }
    }
}
